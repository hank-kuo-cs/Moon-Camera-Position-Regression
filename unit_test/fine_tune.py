...
def fine_tune():
    ...
    target_image = load_image('target.png').to(device)

    target_images = [target_image]
    gt_positions = torch.tensor([[10 / 15, 0 / 180 * 2, 0 / 180 / 2]]).to(device)
    predict_positions = torch.tensor([[11 / 15, 1 / 180 * 2, 1 / 180 / 2]]).to(device)
